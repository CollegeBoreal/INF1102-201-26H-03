...
Write-Output ""
Write-Output "|:hash:| Boréal :id:                | README.md | images | analyse_nginx.ps1 | analyse_nginx.py | VM | <image src='https://avatars0.githubusercontent.com/u/62551735?s=460&v=4' width=20 height=20></image> SSH |"
Write-Output "|------|----------------------------|-----------|--------|---------|----|-----|"
...
for ($g = 0; $g -lt $ACTIVE_GROUP.Count; $g++) {
    ...
    if ($VM_STATUS.ContainsKey($StudentID)) {
        if ($VM_STATUS[$StudentID] -eq "running") {
            ...
            $SSH_TEST = ssh `
                ...
                -i ~/.ssh/${PK_PROF} `
                ubuntu@${ServerID} "ls ~/REGEX/analyse_nginx.* ~/REGEX/rapport_nginx_p*" 2>$null
    ...
    Write-Output "| $i | [$StudentID](../$FILE) :point_right: $URL | $README_STATUS | $IMAGES_STATUS | $TF_STATUS | $PY_STATUS | $VM | $SSH |"
