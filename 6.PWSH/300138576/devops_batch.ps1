...
$topCPU | ForEach-Object {
    Write-Output "$($_.ProcessName) - CPU: $($_.CPU)" | Tee-Object -FilePath $rapportTxt -Append
}

Write-Output "Top Memory :" | Tee-Object -FilePath $rapportTxt -Append
$topMem = Get-Process | Sort-Object WS -Descending | Select-Object -First 5
$topMem | ForEach-Object {
    Write-Output "$($_.ProcessName) - Memory: $($_.WorkingSet)" | Tee-Object -FilePath $rapportTxt -Append
}
