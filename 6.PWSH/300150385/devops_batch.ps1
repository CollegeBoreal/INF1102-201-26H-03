#!/usr/bin/env pwsh

# =========================
# Batch DevOps PowerShell
# =========================

$rapportTxt = "/devops-batch/rapport.txt"
$rapportJson = "/devops-batch/rapport.json"
$hostname = hostname
$user = whoami
$date = Get-Date

Write-Output "===== RAPPORT DEVOPS =====" | Tee-Object $rapportTxt
Write-Output "Date : $date" | Tee-Object -FilePath $rapportTxt -Append
Write-Output "Utilisateur : $user" | Tee-Object -FilePath $rapportTxt -Append
Write-Output "Machine : $hostname" | Tee-Object -FilePath $rapportTxt -Append
Write-Output "" | Tee-Object -FilePath $rapportTxt -Append

# CPU
Write-Output "Top 5 CPU :" | Tee-Object -FilePath $rapportTxt -Append
$topCPU = Get-Process | Sort-Object CPU -Descending | Select-Object -First 5
foreach ($p in $topCPU) {
    Write-Output ("{0} CPU: {1}" -f $p.ProcessName, $p.CPU) | Tee-Object -FilePath $rapportTxt -Append
}

# RAM
Write-Output "" | Tee-Object -FilePath $rapportTxt -Append
Write-Output "Top 5 RAM :" | Tee-Object -FilePath $rapportTxt -Append
$topMem = Get-Process | Sort-Object WS -Descending | Select-Object -First 5
foreach ($p in $topMem) {
    Write-Output ("{0} RAM: {1}" -f $p.ProcessName, $p.WorkingSet) | Tee-Object -FilePath $rapportTxt -Append
}

# DISQUE
Write-Output "" | Tee-Object -FilePath $rapportTxt -Append
Write-Output "Disque :" | Tee-Object -FilePath $rapportTxt -Append
$disk = df -h
Write-Output $disk | Tee-Object -FilePath $rapportTxt -Append

# SSH
Write-Output "" | Tee-Object -FilePath $rapportTxt -Append
$sshHost = "127.0.0.1"
Write-Output "Test SSH :" | Tee-Object -FilePath $rapportTxt -Append
try {
    $result = ssh -o BatchMode=yes -o ConnectTimeout=5 $sshHost "echo OK" 2>&1
    Write-Output "Resultat : $result" | Tee-Object -FilePath $rapportTxt -Append
} catch {
    Write-Output "SSH echoue" | Tee-Object -FilePath $rapportTxt -Append
}

# JSON
$report = [PSCustomObject]@{
    Date = $date
    User = $user
    Machine = $hostname
    CPU = $topCPU
    Memory = $topMem
    Disk = $disk
}

$report | ConvertTo-Json -Depth 5 | Set-Content $rapportJson

Write-Output "Rapports generes"