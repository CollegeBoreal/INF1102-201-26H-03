...
Write-Output ""
Write-Output "|:hash:| Boréal :id:                | README.md | images | `/devops-batch/devops_batch.ps1` | VM | <image src='https://avatars0.githubusercontent.com/u/62551735?s=460&v=4' width=20 height=20></image> SSH |"
Write-Output "|------|----------------------------|-----------|--------|---------|----|-----|"
...
for ($g = 0; $g -lt $ACTIVE_GROUP.Count; $g++) {
    ...
    $TF_FILE = "$StudentID/devops_batch.ps1"
    ...
    if ($VM_STATUS.ContainsKey($StudentID)) {
        if ($VM_STATUS[$StudentID] -eq "running") {
            ...
            $SSH_TEST = ssh `
                ...
                -i ~/.ssh/${PK_PROF} `
                ubuntu@${ServerID} "ls /devops-batch/devops_batch.ps1 /devops-batch/rapport.txt /devops-batch/rapport.json " 2>$null
